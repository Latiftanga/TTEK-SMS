<script lang="ts">
  ...
  import { isSchoolAdmin, userRole } from '$lib/stores/permissions';
  ...
  <nav class="flex-1 overflow-y-auto py-3 space-y-0.5
    ...
    {#each mainNav as item}
      ...
      <a
        ...
        class="group flex items-center rounded-lg transition-all duration-150 text-sm
      ...
      >
        <svg class="h-[18px] w-[18px] shrink-0 transition-transform duration-150
             ...
             fill="none" stroke="currentColor" stroke-width={active ? '2.1' : '1.6'} viewBox="0 0 24 24">
          {@html item.icon}
        </svg>
        {#if !collapsed}
