<script lang="ts">
  ...
  const { studentId, firstName, lastName, photoUrl, canEdit = true }: Props = $props();
...
<div class="group relative h-16 w-16 shrink-0">
  ...
  {#if canEdit}
    <!-- opacity-0/hidden-until-hover has no touch equivalent — group-hover
       ...
       untappable there. Same fix as StaffPhotoAvatar.svelte: a persistent
       lighter tint at rest, fully opaque on hover/focus; the remove badge
       is always shown outright, deliberately smaller (32px) than the
       module's usual touch-target size since a full 44px target would
       swallow most of a 64px avatar. -->
