...
{#if guardian.has_portal_access}
  ...
  {#if canManage}
    <button onclick={() => confirmRevoke = true} disabled={$revokeMut.isPending}
      aria-label="Revoke parent portal access"
...
{:else if canManage}
  <button onclick={() => $grantMut.mutate()} disabled={$grantMut.isPending}
    aria-label="Grant parent portal access"
    class="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg text-[var(--fg-subtle)] transition hover:bg-[var(--hover)] hover:text-[var(--fg)] disabled:opacity-40">
    <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
