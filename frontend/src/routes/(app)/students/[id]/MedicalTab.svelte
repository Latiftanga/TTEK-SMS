...
<div class="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6">
  <div class="mb-4 flex items-center justify-between">
    ...
    {#if !editing && canEdit}
      <button onclick={startEdit}
        class="flex min-h-[44px] items-center gap-1.5 rounded-xl border border-[var(--border)] px-3 py-1.5 text-xs font-medium text-[var(--fg-muted)] transition hover:bg-[var(--hover)]">
  ...
  {#if !medical && !editing}
    <div class="rounded-xl border border-dashed border-[var(--border)] p-6 text-center">
      ...
      {#if canEdit}
        <button onclick={startEdit}
          class="mt-2 inline-flex min-h-[44px] items-center text-xs font-medium text-[var(--brand)] hover:underline transition">
