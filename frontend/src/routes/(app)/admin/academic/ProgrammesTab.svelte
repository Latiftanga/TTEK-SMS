<script lang="ts">
  ...
  import ProgrammeSummaryPanel from './ProgrammeSummaryPanel.svelte';
        ...
        <tbody class="divide-y divide-[var(--border)]">
          {#each paged as prog (prog.id)}
            ...
            {:else}
              <tr class="transition hover:bg-[var(--bg)]">
                ...
                <td class="px-4 py-2.5 text-right">
                  <div class="flex items-center justify-end gap-1">
                    <button onclick={() => expandedSummaryId = expandedSummaryId === prog.id ? null : prog.id}
                      aria-label="Summary for {prog.name}"
                    ...
                    <button onclick={() => startEdit(prog)} aria-label="Edit {prog.name}"
                      class="inline-flex min-h-[44px] items-center gap-1 rounded-lg px-2 py-1 text-xs font-medium text-[var(--fg-muted)] hover:bg-[var(--bg)] hover:text-[var(--fg)]">
                    ...
                    <button onclick={() => prog.is_active ? (confirmDeactivateProg = { id: prog.id, name: prog.name }) : $updateMut.mutate({ id: prog.id, req: { is_active: true } })} disabled={$updateMut.isPending}
                      aria-label="{prog.is_active ? 'Deactivate' : 'Activate'} {prog.name}"
                      class="inline-flex min-h-[44px] items-center gap-1 rounded-lg px-2 py-1 text-xs font-medium disabled:opacity-40 {prog.is_active ? 'text-red-400 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-950/30' : 'text-green-500 hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-950/30'}">
                      {#if prog.is_active}
              ...
              {#if expandedSummaryId === prog.id}
                <tr>
                  <td colspan="4" class="p-0">
                    <ProgrammeSummaryPanel programmeId={prog.id} />
                  </td>
                </tr>
              {/if}
            {/if}
...
<ConfirmModal
  open={!!confirmDeactivateProg}
  title="Deactivate {confirmDeactivateProg?.name ?? 'programme'}?"
  message="Students will no longer be assignable to this programme. Existing enrollments are unaffected. You can reactivate at any time."
  confirmLabel="Deactivate"
  variant="warning"
  isPending={$updateMut.isPending}
  onConfirm={() => { $updateMut.mutate({ id: confirmDeactivateProg!.id, req: { is_active: false } }); confirmDeactivateProg = null; }}
  onCancel={() => confirmDeactivateProg = null}
/>
