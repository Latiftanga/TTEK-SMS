...
<ConfirmModal
  ...
  message="This will make the selected year the active year school-wide. All academic activities will reference it. The previous current year will be deactivated."
  confirmLabel="Set current"
  variant="warning"
  isPending={$setCurrentYearMut.isPending}
  onConfirm={() => { $setCurrentYearMut.mutate(confirmYearId!); confirmYearId = null; }}
  onCancel={() => confirmYearId = null}
/>
