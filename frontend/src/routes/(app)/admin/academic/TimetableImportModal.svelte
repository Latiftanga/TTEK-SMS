<script lang="ts">
  ...
  import { bulkImportTimetable, type ImportBatchResult } from '$lib/api/timetable';
  ...
  $effect(() => {
    if (!open) { phase = 'upload'; file = null; result = null; return; }
    if (!yearId) {
  ...
  const importMut = createMutation({
    mutationFn: () => bulkImportTimetable(file!, yearId),
    onSuccess: (res) => {
      result = res;
      phase = 'results';
  ...
  function handleFiles(files: FileList | null) {
    ...
    const f = files[0];
    if (!f.name.toLowerCase().endsWith('.csv')) { toast.error('Only .csv files are accepted.'); return; }
    file = f;
  }
    ...
    <div class="flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-2xl">
      ...
      {#if phase === 'upload'}
        <div class="flex shrink-0 items-center justify-between border-b border-[var(--border)] px-6 py-4">
          <h2 class="text-lg font-semibold text-[var(--fg)]">Import Timetable</h2>
        ...
        <div class="space-y-3 overflow-y-auto px-6 py-5">
          ...
          <div class="flex items-start gap-3 rounded-xl border border-[var(--border)] bg-[var(--bg)] p-4">
            ...
            <div class="min-w-0 flex-1">
              <p class="text-sm font-medium text-[var(--fg)]">Academic year</p>
              <select bind:value={yearId}
                class="mt-2 w-full rounded-lg border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-sm text-[var(--fg)]">
          ...
          <!-- Step 3 -->
          <div class="flex items-start gap-3 rounded-xl border border-[var(--border)] bg-[var(--bg)] p-4">
            <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[color-mix(in_srgb,var(--brand)_12%,transparent)] text-xs font-bold text-[var(--brand)]">3</div>
            <div class="min-w-0 flex-1">
              <p class="text-sm font-medium text-[var(--fg)]">Upload the CSV</p>
              ...
              >
                <input type="file" accept=".csv" class="sr-only" onchange={(e) => handleFiles(e.currentTarget.files)} />
                {#if file}
                ...
                {:else}
                  ...
                  <p class="text-sm font-medium text-[var(--fg)]">Drop your .csv here</p>
        ...
        <div class="flex shrink-0 items-center justify-end gap-3 border-t border-[var(--border)] px-6 py-4">
          ...
          <button onclick={() => file && yearId && $importMut.mutate()} disabled={!file || !yearId || $importMut.isPending}
          ...
          <div class="flex items-center gap-3 rounded-xl border p-4
            ...
            <div>
              <p class="text-sm font-semibold {allOk ? 'text-emerald-800 dark:text-emerald-200' : 'text-amber-800 dark:text-amber-200'}">
                {result!.created} of {result!.total_rows} slot{result!.total_rows !== 1 ? 's' : ''} imported
              </p>
              {#if result!.failed > 0}
              ...
              {#if hasWarnings}
                <p class="mt-0.5 text-xs text-amber-600 dark:text-amber-400">
                  {result!.warnings.length} imported with a note — see warnings below
                </p>
              {/if}
            </div>
          </div>
                ...
                <table class="w-full text-xs">
                  <thead class="sticky top-0 bg-amber-50 dark:bg-amber-950/40">
                    <tr class="border-b border-amber-200 dark:border-amber-800">
                      ...
                      <th class="px-3 py-2 text-left font-semibold text-amber-700 dark:text-amber-300">Ref</th>
