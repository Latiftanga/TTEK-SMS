    ...
    <div class="space-y-3">
      {#each $smsConfigsQ.data ?? [] as cfg (cfg.id)}
        <div class="flex items-center gap-4 rounded-xl border border-[var(--border)] bg-[var(--card)] px-5 py-4">
          ...
          {#if cfg.is_active}
            <span class="shrink-0 rounded-full bg-green-50 px-2.5 py-0.5 text-xs font-medium text-green-700 ring-1 ring-inset ring-green-600/20">Active</span>
          {:else}
            <button onclick={() => $activateMut.mutate(cfg.provider)} disabled={$activateMut.isPending}
          ...
          {/if}
          <button onclick={() => $deleteMut.mutate(cfg.provider)} disabled={$deleteMut.isPending} title="Remove provider"
