...
<div class="space-y-4">
  <div class="flex justify-end">
    <button onclick={() => { showForm = !showForm; formError = ''; }}
      class="flex min-h-[44px] items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90"
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        <div class="sm:col-span-2">
          ...
          <select bind:value={form.leave_type}
            class="min-h-[44px] w-full rounded-xl border border-[var(--border)] bg-[var(--bg)] px-3 text-sm text-[var(--fg)] focus:border-[var(--brand)] focus:outline-none">
        ...
        <tbody class="divide-y divide-[var(--border)]">
          {#each ($leaveQ.data ?? []) as l (l.id)}
            <tr class="transition hover:bg-[var(--hover)]">
              <td class="px-4 py-3">
                <p class="font-medium text-[var(--fg)]">{l.leave_type}</p>
                <!-- Always shown, not sm:table-cell-hidden — the date range
                     is exactly what an approver needs to see before making
                     the Approve/Reject call below, more than most other
                     hidden-column cases in this app. -->
                <p class="text-xs text-[var(--fg-muted)]">{fmtDate(l.start_date)} – {fmtDate(l.end_date)}</p>
              </td>
                ...
                {#if l.status === 'PENDING' && !isOwnProfile}
                  <div class="flex flex-wrap items-center gap-1.5">
                    <button onclick={() => review(l.id, 'APPROVED')}
                      ...
                      class="min-h-[44px] rounded-lg border border-emerald-200 px-2.5 text-xs font-medium
                    ...
                    <button onclick={() => review(l.id, 'REJECTED')}
                      disabled={$reviewMut.isPending && reviewingId === l.id}
                      class="min-h-[44px] rounded-lg border border-red-200 px-2.5 text-xs font-medium
