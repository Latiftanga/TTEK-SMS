<script lang="ts">
  ...
  import { addQualification, deleteQualification, type StaffDetail } from '$lib/api/staff';
...
<div class="space-y-4">
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        {#each [
          ...
          <div>
            ...
            <input bind:value={form[f.key as keyof typeof form]} placeholder={f.placeholder}
  ...
  {#if staff.qualifications.length === 0}
    <div class="rounded-xl border border-dashed border-[var(--border)] p-8 text-center">
      <p class="text-sm text-[var(--fg-muted)]">No qualifications recorded yet.</p>
    </div>
  {:else}
    <div class="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--card)]">
      {#each staff.qualifications as q, i (q.id)}
        <div class="flex items-start gap-4 px-5 py-4 {i > 0 ? 'border-t border-[var(--border)]' : ''}">
          ...
          <button onclick={() => $delMut.mutate(q.id)}
            class="shrink-0 rounded-lg p-1.5 text-[var(--fg-muted)] transition hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-950/30">
