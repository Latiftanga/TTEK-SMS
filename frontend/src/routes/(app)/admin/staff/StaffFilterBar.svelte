<script lang="ts">
  import type { StaffCategory } from '$lib/api/staff';
  ...
  const {
    searchInput, onSearchInput, genderFilter, onGenderChange, jobFilter, onJobChange,
    activeOnly, onActiveOnlyChange, categories, total, isPending, hasFilters, onClearFilters,
  }: Props = $props();

  // Starts open if a filter is already active so it's never hidden from
  // view — only reads hasFilters once at mount (see StudentFilterBar.svelte,
  // same pattern), so collapsing it afterwards sticks.
  let filtersOpen = $state(hasFilters);

  const selClass = 'w-full sm:w-auto rounded-xl border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-sm text-[var(--fg)] focus:border-[var(--brand)] focus:outline-none transition';
</script>
...
<div class="mb-4 space-y-2">
  ...
  <div class="flex items-center gap-2">
    <div class="relative min-w-0 flex-1">
      ...
      <input value={searchInput} oninput={(e) => onSearchInput(e.currentTarget.value)} placeholder="Search name, ID, or position…"
  ...
  {#if filtersOpen}
    <div class="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center">
      <select value={genderFilter} onchange={(e) => onGenderChange(e.currentTarget.value)} class={selClass}>
      ...
      {#if categories.length > 0}
        <select value={jobFilter} onchange={(e) => onJobChange(e.currentTarget.value)} class={selClass}>
      ...
      <label class="col-span-2 flex cursor-pointer items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-sm text-[var(--fg-muted)] transition hover:bg-[var(--hover)] sm:col-span-1">
        <input type="checkbox" checked={activeOnly} onchange={(e) => onActiveOnlyChange(e.currentTarget.checked)} class="accent-[var(--brand)]" />
        Active only
      </label>
