...
<div class="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)]">
  ...
  <div class="flex items-center gap-2 px-4 py-3.5">
    ...
    <div class="flex shrink-0 items-center gap-2">
      ...
      <!-- Edit button — title tooltips never show on a touch screen, so
           aria-label (for screen readers) plus a real 44px touch target
           stand in for the missing visible label on this dense admin row. -->
      {#if !isShared}
        <button onclick={startEdit}
          ...
          aria-label="Edit scale" title="Edit scale">
        ...
        <button
          ...
          class="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-lg transition disabled:opacity-40 {scale.is_active ? 'text-[var(--fg-subtle)] hover:bg-amber-50 hover:text-amber-600 dark:hover:bg-amber-950/30' : 'text-green-500 hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-950/30'}"
          aria-label={scale.is_active ? 'Deactivate' : 'Activate'} title={scale.is_active ? 'Deactivate' : 'Activate'}>
          {#if scale.is_active}
            <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
