      ...
      <div class="mb-4 rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
        <div class="grid gap-3 sm:grid-cols-3">
          ...
          <div>
            ...
            <input type="number" min="1" bind:value={af.capacity} placeholder="e.g. 8" class="input" />
          </div>
          <div>
            ...
            {#if editId === r.id}
              <tr class="border-b border-[var(--border)] bg-[var(--hover)]">
                ...
                <td class="px-3 py-2 text-right">
                  <input type="number" min="1" bind:value={ef.capacity} class="input w-20 text-right" />
                </td>
                <td class="px-3 py-2">
                  <div class="flex gap-1">
                    <button onclick={() => $editMut.mutate()} disabled={$editMut.isPending}
                      class="rounded-lg px-2 py-1 text-xs font-semibold text-white disabled:opacity-50"
                      style="background:var(--brand)">Save</button>
                    <button onclick={() => editId = null}
                      class="rounded-lg border border-[var(--border)] px-2 py-1 text-xs text-[var(--fg-muted)] hover:bg-[var(--hover)]">
                      Cancel
                    </button>
                  </div>
                </td>
                ...
                {#if canManage}
                  <td class="px-4 py-3 text-right">
                    <button onclick={() => startEdit(r)}
                      class="text-xs text-[var(--fg-subtle)] hover:text-[var(--fg)] transition">Edit</button>
                  </td>
                {/if}
              </tr>
            {/if}
