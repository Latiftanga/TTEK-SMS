...
async def subject(db_session: AsyncSession, school: School, school_class: Class) -> Subject:
    ...
    db_session.add(cat)
    await db_session.flush()
    subj = Subject(school_id=school.id, code="ENG_SCOPE", name="English", is_active=True)
    db_session.add(subj)
    await db_session.flush()
...
async def test_list_assessments_filters_to_taught_subject_only(
    ...
    db_session.add(cat2)
    await db_session.flush()
    other_subject = Subject(school_id=school.id, code="MATH_SCOPE", name="Maths", is_active=True)
