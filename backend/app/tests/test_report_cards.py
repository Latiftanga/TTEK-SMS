...
async def test_assemble_groups_scores_by_subject_with_total_and_grade(
    ...
    db_session.add(cat)
    await db_session.flush()
    subject = Subject(school_id=school.id, code="MATH_RC", name="Mathematics", is_active=True)
...
async def test_assemble_excludes_diagnostic_category_and_averages_multi_entry_type(
    ...
    db_session.add(cat)
    await db_session.flush()
    subject = Subject(school_id=school.id, code="SCI_DIAG", name="Integrated Science", is_active=True)
    db_session.add(subject)
    await db_session.flush()
    db_session.add(ClassSubject(school_id=school.id, class_id=school_class.id, subject_id=subject.id, is_active=True))
    await db_session.flush()
