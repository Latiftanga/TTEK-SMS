...
The template contains:
  ...
  - Rows 5-204: Pre-formatted data-entry area with:
      · Gender dropdown (MALE / FEMALE)
      · Position dropdown (populated from DB positions)
      · Alternating row fill for readability
      · Unlocked cells so the sheet protection doesn't block editing
  - Sentinel cell N1 = "TTEK_STAFF_IMPORT_V1" (validated by importer)
...
from app.services.staff_import_constants import _COLS, _DATA_START, _NUM_ROWS, make_sentinel
...
def build_template(school_name: str, school_code: str, brand_color: str, position_names: list[str]) -> bytes:
    ...
    sample_vals = [
        ...
        "GHA-001234567", "0244000000", "k.mensah@school.edu.gh",
        position_names[0] if position_names else "", "Science", "2020-09-01",
    ]
    ...
    if position_names:
        ...
        dv_pos.sqref = f"J{_DATA_START}:J{_DATA_START + _NUM_ROWS - 1}"
...
def _add_instructions_sheet(wb: Workbook, brand_fill: PatternFill, hdr_font: Font) -> None:
    ...
    rows = [
        ...
        ("Department",     "e.g. Science, Mathematics, Administration.",              False),
